...
o = s:option(Value, "auto_update_time", translate("Update time (every day)"))
...
o = s:option(DummyValue, "type", translate("Node Type"))
function o.cfgvalue(self, section)
    return m:get(section, "v2ray_protocol") or Value.cfgvalue(self, section) or translate("None")
end

o = s:option(DummyValue, "alias", translate("Node Alias"))
function o.cfgvalue(...)
...
node = s:option(Button, "apply_node", translate("Select Node"))
node.inputstyle = "apply"
node.render = function(self, section, scope)
    if section == global_server then
        self.title = translate("Current Node")
    else
        self.title = translate("Select Node")
    end
